...
async def list_callbacks(
    status: str | None = Query(
        None,
        description="Filter by status (e.g. pending, completed, failed, cancelled)",
    ),
    source: str = Query(
        "all", description="Source of callback: all, standalone, campaign"
    ),
    ...
    async with db_client.async_session() as db:
        if source in ("all", "standalone") and not campaign_id:
            q_sa = select(ScheduledCallbackModel).where(
                ScheduledCallbackModel.organization_id == org_id
            )
            if status:
            ...
            q_sa = q_sa.order_by(desc(ScheduledCallbackModel.scheduled_for)).limit(
                offset + limit
            )
        ...
        if source in ("all", "campaign"):
            q_camp = (
                select(QueuedRunModel, CampaignModel)
                .join(CampaignModel, QueuedRunModel.campaign_id == CampaignModel.id)
                .where(
            ...
            q_camp = q_camp.order_by(desc(QueuedRunModel.scheduled_for)).limit(
                offset + limit
            )
    ...
    merged.sort(
        key=lambda x: x.scheduled_for or datetime.min.replace(tzinfo=UTC), reverse=True
    )
    ...
    return UnifiedCallbackListResponse(
        items=paginated, total=total, has_more=len(merged) > offset + limit
    )
...
async def cancel_callback(
    ...
    if source not in ("standalone", "campaign"):
        raise HTTPException(
            status_code=400, detail="Invalid source. Must be 'standalone' or 'campaign'"
        )
    ...
    async with db_client.async_session() as db:
        if source == "campaign":
            ...
            if qr.state != "queued":
                raise HTTPException(
                    status_code=400,
                    detail=f"Cannot cancel a callback in {qr.state} state",
                )
            ...
            if callback.status != "pending":
                raise HTTPException(
                    status_code=400,
                    detail=f"Cannot cancel a callback in {callback.status} state",
                )
