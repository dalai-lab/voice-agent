...
import { Phone, PhoneForwarded, Percent } from 'lucide-react';
...
export function MetricsCards({ metrics }: MetricsCardsProps) {
  const transferRate = metrics.total_runs > 0
    ? ((metrics.xfer_count / metrics.total_runs) * 100).toFixed(1)
    : '0.0';

  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      <div className="p-4 rounded-xl border border-border/60 bg-card hover:bg-card/80 transition-all duration-200 shadow-xs flex items-center justify-between text-card-foreground">
      ...
      <div className="p-4 rounded-xl border border-border/60 bg-card hover:bg-card/80 transition-all duration-200 shadow-xs flex items-center justify-between text-card-foreground">
        ...
        <div className="w-9 h-9 rounded-lg bg-emerald-500/10 flex items-center justify-center text-emerald-500 border border-emerald-500/15">
      ...
      <div className="p-4 rounded-xl border border-border/60 bg-card hover:bg-card/80 transition-all duration-200 shadow-xs flex items-center justify-between text-card-foreground">
        ...
        <div className="w-9 h-9 rounded-lg bg-indigo-500/10 flex items-center justify-center text-indigo-500 border border-indigo-500/15">
