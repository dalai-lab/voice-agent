...
const PUBLIC_PATHS = ['/auth/login', '/auth/signup', '/auth/impersonate', '/embed', '/onboardingdemo'];
...
export async function middleware(request: NextRequest) {
  ...
  // For Stack Auth (and unknown), Next.js middleware has no way to validate the
  // session token — the Stack Auth opaque access token cannot be verified from
  // the Edge runtime. Talkar account gating is handled client-side in AppLayout.
  if (authProvider !== 'local') {
    return NextResponse.next();
  }

  // Local auth: redirect to login if no session cookie present
  if (!token) {
